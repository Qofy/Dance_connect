...
function main() {
  ...
  if [ ${_err} -gt 0 ] ; then
  {
      ...
      echo "FAILED UPLOADS:
      ---------------- uploads_failed >>>
       knockscp.sh velotermin.bike velotermin_bike ./"backend.tar.gz" "/home/zeus/_/software/mitote.intuivo.com/backend.tar.gz"
      >>>>>>>>>>>> ------
      ${msg}
      ---------------- <<< uploads_failed: ${_err}  "
  ...
  function main_ejecutar() {
    ...
    set -x
    msg="$(curl $passwords -k $service_url/pass_backend_to_server_mitote.php)"
    _err=$?
    set +x
